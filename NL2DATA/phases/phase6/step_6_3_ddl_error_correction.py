"""Phase 6, Step 6.3: DDL Error Correction.

If DDL validation fails, use LLM to analyze errors and generate IR patches.
Only invoked when Step 10.2 reports validation failures.
...
@traceable_step("6.3", phase=10, tags=['phase_10_step_3'])
async def step_6_3_ddl_error_correction(
    validation_errors: Dict[str, Any],  # Output from Step 10.2
    original_ddl: List[str],  # Original DDL statements
    normalized_schema: Dict[str, Any],  # Full normalized schema
    relational_schema: Optional[Dict[str, Any]] = None,  # Relational schema
    ...
    Args:
        validation_errors: Validation result from Step 10.2 with syntax_errors, naming_conflicts
    ...
    Example:
        ...
        >>> result = await step_6_3_ddl_error_correction(errors, ["..."], {...})
        >>> len(result["ir_patches"]) > 0
        True
    """
    logger.info("Starting Step 10.3: DDL Error Correction (LLM, conditional)")
    ...
    if not error_summary:
        ...
        return DDLErrorCorrectionOutput(
            ir_patches=[],
            corrections=[],
    ...
    output_structure_section = generate_output_structure_section_with_custom_requirements(
        ...
        additional_requirements=[
            "You must output IR patches (changes to LogicalIR), NOT direct DDL edits",
            "The DDL will be regenerated deterministically from the patched IR",
            "For each patch, provide operation, target, changes, and reasoning",
        ]
    )
    
    # Create prompt
    system_prompt = f"""You are a database schema expert. Your task is to analyze DDL validation errors and generate patches to fix the LogicalIR schema.
