"""Phase 5, Step 5.5: SQL Query Generation.
...
async def step_5_5_sql_query_generation(
    ...
    data_types: Optional[Dict[str, Dict[str, Dict[str, Any]]]] = None,  # Data types from Step 4.3
    ...
    Args:
        information_need: Information need from Step 3.1 with description, entities_involved
    ...
    Example:
        ...
        >>> result = await step_5_5_sql_query_generation(info_need, {...})
    ...
    # Create prompt
    system_prompt = """You are a SQL query generation expert. Your task is to convert natural language information needs into executable SQL queries.
...
After using validation tools (if needed), you MUST return ONLY a valid JSON object with this exact structure:
{
...
1. The "sql" field MUST ALWAYS be a valid SQL query string - NEVER null, NEVER empty, NEVER missing
2. Even if validation fails, you MUST still provide a valid SQL query in the "sql" field
3. If validation fails, set "validation_status" to "invalid" and provide a corrected version in "corrected_sql" if possible
4. The "sql" field is REQUIRED and must be a non-empty string - it cannot be null or undefined
...
- Returns the information described in the need"""
    ...
    try:
        config = get_trace_config("5.5", phase=5, tags=["sql_generation"])
...
async def step_5_5_sql_query_generation_batch(
