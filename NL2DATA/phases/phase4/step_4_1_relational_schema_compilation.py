"""Phase 4, Step 4.1: Relational Schema Compilation.
...
class RelationalSchemaCompilationOutput(BaseModel):
    """Output structure for relational schema compilation."""
    relational_schema: RelationalSchema = Field(
        description="Compiled relational schema with tables, columns, primary keys, and foreign keys"
    )
...
def step_4_1_relational_schema_compilation(
    er_design,  # ER design from Step 3.1 (dict or ERDesignCompilationOutput)
    foreign_keys: List,  # Foreign keys from Phase 2 (legacy, may be used for reference)
    primary_keys,  # entity -> PK from Phase 2 (Dict[str, List[str]] or similar)
    constraints: Optional[List] = None,  # Constraints from Phase 2
    junction_table_names: Optional = None,  # relation_key -> suggested table name from Step 3.2 (dict or JunctionTableNamingOutput)
) -> RelationalSchemaCompilationOutput:
    """
    Step 4.1 (deterministic): Compile relational schema from ER design.
    ...
    Args:
        er_design: ER design from Step 3.1 with entities, relations, attributes
    ...
    Example:
        >>> schema = step_4_1_relational_schema_compilation(
        ...
        True
    """
    logger.info("Starting Step 4.1: Relational Schema Compilation (deterministic)")
    
    # Convert er_design to dict if it's a Pydantic model
    if hasattr(er_design, 'model_dump'):
        er_design_dict = er_design.model_dump()
        # Convert entity_attributes list back to dict format if needed
        if "entity_attributes" in er_design_dict and "attributes" not in er_design_dict:
            er_design_dict["attributes"] = {entry["entity_name"]: entry["attributes"] for entry in er_design_dict["entity_attributes"]}
    else:
        er_design_dict = er_design
    
    # Convert primary_keys to dict if needed
    if not isinstance(primary_keys, dict):
        primary_keys = dict(primary_keys) if hasattr(primary_keys, '__iter__') else {}
    
    # Convert junction_table_names to dict if needed
    if junction_table_names:
        if hasattr(junction_table_names, 'junction_table_names'):
            # It's a JunctionTableNamingOutput
            junction_dict = {entry.relation_key: entry.table_name for entry in junction_table_names.junction_table_names}
        elif hasattr(junction_table_names, 'model_dump'):
            # It's a dict-like Pydantic model
            jtn_dict = junction_table_names.model_dump()
            if "junction_table_names" in jtn_dict:
                junction_dict = {entry["relation_key"]: entry["table_name"] for entry in jtn_dict["junction_table_names"]}
            else:
                junction_dict = jtn_dict
        else:
            junction_dict = junction_table_names
    else:
        junction_dict = {}
    
    # Validate ER design via Pydantic (avoid raw dict usage internally)
    er_model = ERDesign.model_validate(er_design_dict)
    ...
    for relation in relations:
        ...
        if relation_arity == 2:
            ...
            if e1_cardinality == "1" and e2_cardinality == "1":
                ...
                # Check if we have a suggested name from Step 3.2
                relation_key = _get_relation_key([e1_name, e2_name])
                if junction_dict and relation_key in junction_dict:
                    junction_table_name = junction_dict[relation_key]
                else:
    ...
    # Convert tables to Pydantic Table objects
    pydantic_tables = []
    for t in tables:
        pydantic_tables.append(_to_pydantic_table(t))
    
    relational_schema = RelationalSchema(tables=pydantic_tables)
    
    return RelationalSchemaCompilationOutput(relational_schema=relational_schema)
