"""Phase 8, Step 8.1: Functional Dependency Analysis.
...
from NL2DATA.utils.pipeline_config import get_phase4_config as get_phase7_config
...
@traceable_step("8.1", phase=8, tags=['phase_8_step_1'])
async def step_8_1_functional_dependency_analysis_single(
    entity_name: str,
    ...
    """
    Step 7.1 (per-entity): Identify functional dependencies for an entity.
    ...
    output_structure_section = generate_output_structure_section_with_custom_requirements(
        ...
        additional_requirements=[
            "Every functional dependency object in functional_dependencies, should_add, and should_remove MUST include a 'reasoning' field",
            "The reasoning field cannot be omitted, left empty, or set to a placeholder value",
            "Do NOT include functional dependencies where the left-hand side is the primary key (or contains the primary key) determining all other attributes",
            "EXCLUDE PK → all dependencies (always true, not useful for normalization)",
        ]
    )
    
    # System prompt
    system_prompt = f"""You are a database normalization expert. Your task is to identify functional dependencies between attributes.
...
Return a JSON object specifying all functional dependencies, any additions or removals from previous iterations, and whether you're satisfied with the current list."""
    ...
    try:
        ...
        for reasoning_attempt in range(max_reasoning_retries + 1):
            try:
                config = get_trace_config("8.1", phase=7, tags=["phase_8_step_1"])
                nl_section = ""
...
async def step_8_1_functional_dependency_analysis_single_with_loop(
    entity_name: str,
    entity_description: Optional[str],
    ...
    Example:
        >>> result = await step_8_1_functional_dependency_analysis_single_with_loop(
        ...     entity_name="Customer",
    ...
    async def fd_analysis_step(previous_result=None):
        ...
        if previous_result:
            if hasattr(previous_result, 'functional_dependencies'):
                previous_dependencies = [fd.model_dump() if hasattr(fd, 'model_dump') else fd for fd in previous_result.functional_dependencies]
            else:
    ...
    if hasattr(final_result, 'model_dump'):
        ...
        deps_count = len(final_result.functional_dependencies)
    else:
        final_result_dict = final_result
        deps_count = len(final_result.get('functional_dependencies', []))
    
    logger.info(
        f"Functional dependency analysis loop for {entity_name} completed: {iterations} iterations, "
        f"terminated by: {terminated_by}, {deps_count} dependencies identified"
    )
...
async def step_8_1_functional_dependency_analysis_batch(
    entities: List[Dict[str, Any]],  # All entities with descriptions
    entity_attributes: Optional[Dict[str, List[Dict[str, Any]]]] = None,  # entity -> attributes
    entity_primary_keys: Optional[Dict[str, List[str]]] = None,  # entity -> primary key
    relational_schema: Optional[Dict[str, Any]] = None,  # Relational schema from Step 4.1/Phase 4 (preferred source for FD mining)
    ...
    """
    Step 7.1: Analyze functional dependencies for all tables/entities (parallel execution).
    ...
    - Functional dependencies should ideally be mined AFTER ER -> relational schema compilation (Phase 4),
      because normalization operates on relational tables (including FKs, junction tables, and inferred PKs).
    ...
    Args:
        ...
        relational_schema: Optional relational schema from Phase 4 (preferred FD mining source)
    ...
    Example:
        >>> result = await step_8_1_functional_dependency_analysis_batch(
    ...
    if relational_schema and relational_schema.get("tables"):
        logger.info(
            f"Starting Step 7.1: Functional Dependency Analysis for {len(relational_schema.get('tables', []))} tables "
            f"(using relational schema as source of truth)"
        )
    else:
        logger.info(f"Starting Step 7.1: Functional Dependency Analysis for {len(entities)} entities")
    ...
    if relational_schema and relational_schema.get("tables"):
        ...
        # This aligns with normalization (which operates on tables).
        for table in relational_schema.get("tables", []):
            ...
            task = step_8_1_functional_dependency_analysis_single_with_loop(
        ...
        for entity in entities:
            ...
            attributes = (entity_attributes or {}).get(entity_name, [])
    ...
    for i, ((entity_name, _), result) in enumerate(zip(tasks, results)):
        if isinstance(result, Exception):
            ...
            entity_results[entity_name] = final_result_dict
            # Collect all functional dependencies for the top-level list
            fds = final_result_dict.get("functional_dependencies", [])
            for fd in fds:
                if isinstance(fd, dict):
                    all_functional_dependencies.append(fd)
                else:
                    all_functional_dependencies.append(fd.model_dump() if hasattr(fd, "model_dump") else fd)
    ...
    return {
        ...
        "functional_dependencies": all_functional_dependencies,
    }
