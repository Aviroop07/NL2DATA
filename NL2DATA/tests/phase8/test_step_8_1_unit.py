"""Unit tests for Step 8.1: Functional Dependency Analysis."""
...
async def test_step_8_1_basic():
    """Test Step 8.1 with basic input."""
    TestResultDisplay.print_test_header("Functional Dependency Analysis", "8.1")
    ...
    with LLMMockHelper.patch_standardized_llm_call(mock_response):
        with LLMMockHelper.patch_model_router("8.1"):
    ...
    TestResultDisplay.print_test_summary(all_valid, "8.1")
    return all_valid


async def main():
    """Run all tests for Step 8.1."""
    results = []
    results.append(await test_step_8_1_basic())
    
    print("\n" + "=" * 80)
    if all(results):
        print("[PASS] All Step 8.1 unit tests passed!")
    else:
        print("[FAIL] Some Step 8.1 unit tests failed")
    print("=" * 80)
